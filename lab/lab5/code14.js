...
{
    cube = new Cube(1000);
    sphere = new Sphere(8, 20,20);
}

function drawObjects(obj, is_object)
{
         setMatrixUniforms();
      gl.uniform1i(shaderProgram.shouldDrawObject, is_object);
...
function initCubeMap() {
   ...
   // cubemapTexture.generateMipmaps = false;
    gl.bindTexture(gl.TEXTURE_CUBE_MAP, cubemapTexture);
    gl.texParameteri(gl.TEXTURE_CUBE_MAP, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE); 
    gl.texParameteri(gl.TEXTURE_CUBE_MAP, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
    gl.texParameteri(gl.TEXTURE_CUBE_MAP, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
    gl.texParameteri(gl.TEXTURE_CUBE_MAP, gl.TEXTURE_MIN_FILTER, gl.LINEAR); 
    var faces = [["images/pos-x.png", gl.TEXTURE_CUBE_MAP_POSITIVE_X],
    ...
    for(var i = 0; i < faces.length; i++)
        {
          ...
          var face = faces[i][1];
          var image = new Image();
          image.onload = function(cubemapTexture, face, image){
                return function(){
                   gl.bindTexture(gl.TEXTURE_CUBE_MAP, cubemapTexture);
                        gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, false);
                        gl.texImage2D(face, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, image);
                }
    }(cubemapTexture, face, image);
            image.src = faces[i][0];   
...
function initTextures() {
    ...
    sampleTexture.image.src = "images/vase.jpg";    
...
if(use_texture == 2)
    {
          drawObjects(cube, false);
            gl.drawElements(gl.TRIANGLES, teapotVertexIndexBuffer.numItems , gl.UNSIGNED_SHORT, 0); 
          drawObjects(sphere, true);
